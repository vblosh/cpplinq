...
TEST_F(MssqlIntegrationTest, UpsertEntity) {
...
TEST_F(MssqlIntegrationTest, SetOperations) {
...
TEST_F(MssqlIntegrationTest, ConnectionPoolWithMssql) {
    PoolConfig config;
    config.min_connections = 2;
    config.max_connections = 4;

    auto pool = make_pool<mssql>(conn_str_, config);
...
TEST_F(MssqlIntegrationTest, SubqueriesExistsAndIn) {
...
TEST_F(MssqlIntegrationTest, BulkOperationsAndChunking) {
    if (!db) GTEST_SKIP() << "MSSQL test instance not available";
...
TEST_F(MssqlIntegrationTest, DataTypingRoundTripAndQueries) {
    if (!db) return;
