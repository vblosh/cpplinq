...
protected:
    using backend_type = typename Params::backend_type;

    void SetUp() override {
        if (!Params::is_enabled()) {
            std::cout << "[SKIPPED] " << Params::name << " integration tests skipped: " << Params::skip_reason() << std::endl;
            GTEST_SKIP() << Params::skip_reason();
            return;
        }

        try {
            conn_str_ = Params::connection_string();
            db = std::make_unique<DbContext<backend_type>>(conn_str_);
        } catch (const std::exception& e) {
            std::cout << "[SKIPPED] " << Params::name << " integration tests skipped: Could not connect (" << e.what() << ")" << std::endl;
            GTEST_SKIP() << "Could not connect to " << Params::name << ": " << e.what();
            return;
        }

        try {
            Params::clean_tables(*db);
            ...
            std::cout << "[SKIPPED] " << Params::name << " table setup failed: " << e.what() << std::endl;
            GTEST_SKIP() << Params::name << " table setup failed: " << e.what();
        }
    }
    ...
    std::unique_ptr<DbContext<backend_type>> db;
};

TYPED_TEST_SUITE_P(IntegrationTestSuite);

TYPED_TEST_P(IntegrationTestSuite, EnsureTableAndInsertSingle) {
    auto& db = this->db;
    if (!db) return;
...
TYPED_TEST_P(IntegrationTestSuite, ConnectWithDsnPrefix) {
    using backend_type = typename TypeParam::backend_type;
    auto dsn_db = cpplinq::connect<backend_type>(this->conn_str_);
    EXPECT_TRUE(dsn_db.connection().is_open());
    dsn_db.ensure_table(users_table);
    EXPECT_EQ(dsn_db.from(users_table).count(), 0);
}

TYPED_TEST_P(IntegrationTestSuite, InsertMany) {
    auto& db = this->db;
    if (!db) return;
...
TYPED_TEST_P(IntegrationTestSuite, QueryWhereFilter) {
    auto& db = this->db;
    if (!db) return;
...
TYPED_TEST_P(IntegrationTestSuite, QueryOrderByAndLimitOffset) {
    auto& db = this->db;
    if (!db) return;
    ...
    std::vector<User> page = db->from(users_table)
...
TYPED_TEST_P(IntegrationTestSuite, QueryFirst) {
    auto& db = this->db;
    if (!db) return;
...
TYPED_TEST_P(IntegrationTestSuite, Aggregates) {
    auto& db = this->db;
    if (!db) return;
...
TYPED_TEST_P(IntegrationTestSuite, UpdateRecords) {
    auto& db = this->db;
    if (!db) return;
...
TYPED_TEST_P(IntegrationTestSuite, DeleteRecords) {
    auto& db = this->db;
    if (!db) return;
...
TYPED_TEST_P(IntegrationTestSuite, TransactionCommit) {
    auto& db = this->db;
    if (!db) return;
...
TYPED_TEST_P(IntegrationTestSuite, TransactionRollback) {
    auto& db = this->db;
    if (!db) return;
...
TYPED_TEST_P(IntegrationTestSuite, NullAndNotNullFilters) {
    auto& db = this->db;
    if (!db) return;
...
TYPED_TEST_P(IntegrationTestSuite, BetweenAndLikeFilters) {
    auto& db = this->db;
    if (!db) return;
...
TYPED_TEST_P(IntegrationTestSuite, InListAndMultiColumnThenBy) {
    auto& db = this->db;
    if (!db) return;
...
TYPED_TEST_P(IntegrationTestSuite, SqlFunctionsFilter) {
    auto& db = this->db;
    if (!db) return;
...
TYPED_TEST_P(IntegrationTestSuite, GroupByAndCountDistinct) {
    auto& db = this->db;
    if (!db) return;
...
TYPED_TEST_P(IntegrationTestSuite, InnerJoinAndLeftJoin) {
    auto& db = this->db;
    if (!db) return;
...
TYPED_TEST_P(IntegrationTestSuite, UpsertEntity) {
    auto& db = this->db;
    if (!db) return;
...
TYPED_TEST_P(IntegrationTestSuite, SetOperations) {
    auto& db = this->db;
    if (!db) return;
...
TYPED_TEST_P(IntegrationTestSuite, ConnectionPool) {
    using backend_type = typename TypeParam::backend_type;
    PoolConfig config;
    config.min_connections = 2;
    config.max_connections = 4;

    auto pool = make_pool<backend_type>(this->conn_str_, config);
    EXPECT_EQ(pool->size(), 2);

    pool->with_context([&](auto& db_ctx) {
        db_ctx.ensure_table(users_table);
...
TYPED_TEST_P(IntegrationTestSuite, SubqueriesExistsAndIn) {
    auto& db = this->db;
    if (!db) return;
...
TYPED_TEST_P(IntegrationTestSuite, DateTimeFunctions) {
    auto& db = this->db;
    if (!db) return;
...
TYPED_TEST_P(IntegrationTestSuite, WindowFunctions) {
    auto& db = this->db;
    if (!db) return;
    ...
    std::string q_name = db->connection().dialect().quote_id("name");
    std::string q_dept = db->connection().dialect().quote_id("department");
    std::string q_sal = db->connection().dialect().quote_id("salary");
    std::string q_tbl = db->connection().dialect().quote_id("test_employees");

    std::string sql = "SELECT " + q_name + ", " + q_dept + ", " + q_sal +
                      ", ROW_NUMBER() OVER (PARTITION BY " + q_dept + " ORDER BY " + q_sal + " DESC) as rn FROM " +
                      q_tbl + " ORDER BY " + q_dept + ", rn;";

    auto prep = db->connection().prepare(sql);
...
TYPED_TEST_P(IntegrationTestSuite, MultiTableJoins) {
    auto& db = this->db;
    if (!db) return;
...
TYPED_TEST_P(IntegrationTestSuite, CommonTableExpressions) {
    auto& db = this->db;
    if (!db) return;
    ...
    std::string q_id = db->connection().dialect().quote_id("id");
    std::string q_name = db->connection().dialect().quote_id("name");
    std::string q_age = db->connection().dialect().quote_id("age");
    std::string q_tbl = db->connection().dialect().quote_id("test_users");

    std::string sql = "WITH active_adults AS (SELECT " + q_id + ", " + q_name + ", " + q_age +
                      " FROM " + q_tbl + " WHERE " + q_age + " >= 25) " +
                      "SELECT " + q_id + ", " + q_name + ", " + q_age +
                      " FROM active_adults WHERE " + q_age + " >= 30 ORDER BY " + q_name + ";";
...
TYPED_TEST_P(IntegrationTestSuite, BulkOperationsAndChunking) {
    auto& db = this->db;
    if (!db) return;
...
TYPED_TEST_P(IntegrationTestSuite, DataTypingRoundTripAndQueries) {
    auto& db = this->db;
    if (!db) return;
    ...
    EXPECT_TRUE(all_measurements[0].localized_label == L"Sensor \u03a9 Alpha" || all_measurements[0].localized_label == L"Sensor O Alpha");
    ...
    EXPECT_TRUE(found_guid->localized_label == L"Sensor \u03a9 Alpha" || found_guid->localized_label == L"Sensor O Alpha");
...
TYPED_TEST_P(IntegrationTestSuite, PreparedStatementReuseRaw) {
    auto& db = this->db;
    if (!db) return;
    ...
    std::string sql = "SELECT " + q_name + ", " + q_age + " FROM " + q_tbl + " WHERE " + q_age + " >= " + db->connection().dialect().placeholder(0) + " ORDER BY " + q_age + " ASC";
    auto stmt = db->connection().prepare(sql);
...
TYPED_TEST_P(IntegrationTestSuite, PreparedQueryExecutionAndReuse) {
    auto& db = this->db;
    if (!db) return;
...
TYPED_TEST_P(IntegrationTestSuite, PreparedQueryMultipleAndMixedParams) {
    auto& db = this->db;
    if (!db) return;
    ...
    auto pq = db->from(employees_table)
                 ...
                 .template prepare<std::string, int>();
...
TYPED_TEST_P(IntegrationTestSuite, PreparedQueryStreaming) {
    auto& db = this->db;
    if (!db) return;
...
TYPED_TEST_P(IntegrationTestSuite, PreparedCommandUpdateAndRemove) {
    auto& db = this->db;
    if (!db) return;
...
TYPED_TEST_P(IntegrationTestSuite, DbContextBatchPreparedReuse) {
    auto& db = this->db;
    if (!db) return;
...
REGISTER_TYPED_TEST_SUITE_P(
    ...
    EnsureTableAndInsertSingle,
    ConnectWithDsnPrefix,
    InsertMany,
    QueryWhereFilter,
    QueryOrderByAndLimitOffset,
    QueryFirst,
    Aggregates,
    UpdateRecords,
    DeleteRecords,
    TransactionCommit,
    TransactionRollback,
    NullAndNotNullFilters,
    BetweenAndLikeFilters,
    InListAndMultiColumnThenBy,
    SqlFunctionsFilter,
    GroupByAndCountDistinct,
    InnerJoinAndLeftJoin,
    UpsertEntity,
    SetOperations,
    ConnectionPool,
    SubqueriesExistsAndIn,
    DateTimeFunctions,
    WindowFunctions,
    MultiTableJoins,
    CommonTableExpressions,
    BulkOperationsAndChunking,
    DataTypingRoundTripAndQueries,
    PreparedStatementReuseRaw,
    PreparedQueryExecutionAndReuse,
    PreparedQueryMultipleAndMixedParams,
    PreparedQueryStreaming,
    PreparedCommandUpdateAndRemove,
    DbContextBatchPreparedReuse
);
