...
protected:
    void SetUp() override {
        ...
        try {
            ...
            std::cout << "[SKIPPED] MySQL table setup failed: " << e.what() << std::endl;
            GTEST_SKIP() << "MySQL table setup failed: " << e.what();
        }
    }
...
TEST_F(MysqlIntegrationTest, BulkOperationsAndChunking) {
    if (!db) GTEST_SKIP() << "MySQL test instance not available";

    // 1. Array batch insert with chunking
    ...
    // 3. update_many with ODBC array binding
...
TEST_F(MysqlIntegrationTest, DataTypingRoundTripAndQueries) {
    if (!db) return;
    ...
    m1.localized_label = L"MySQL \u03a9 Alpha";
    ...
    EXPECT_EQ(all_measurements[0].localized_label, L"MySQL \u03a9 Alpha");
    ...
    EXPECT_EQ(found_guid->localized_label, L"MySQL \u03a9 Alpha");
    ...
    found_date->localized_label = L"Updated MySQL";
    ...
    EXPECT_EQ(reloaded->localized_label, L"Updated MySQL");
}
