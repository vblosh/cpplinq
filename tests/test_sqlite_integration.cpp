...
inline const auto orders_table = table<Order>(
    ...
    column("user_id", &Order::user_id),
    column("amount", &Order::amount)
);
...
protected:
    void SetUp() override {
        db = std::make_unique<DbContext<sqlite>>(":memory:");
...
TEST_F(SqliteIntegrationTest, EnsureTableAndInsertSingle) {
    ...
    auto all = db->from(users_table).to_list();
...
TEST_F(SqliteIntegrationTest, InsertMany) {
...
TEST_F(SqliteIntegrationTest, QueryWhereFilter) {
...
TEST_F(SqliteIntegrationTest, QueryOrderByAndLimitOffset) {
    ...
    // Descending order with limit and offset, also testing .to_list().to_vector()
...
TEST_F(SqliteIntegrationTest, QueryFirst) {
    db->insert(users_table, User{0, "Alice", "alice@test.com", 30});
...
TEST_F(SqliteIntegrationTest, Aggregates) {
...
TEST_F(SqliteIntegrationTest, UpdateRecords) {
...
TEST_F(SqliteIntegrationTest, DeleteRecords) {
...
TEST_F(SqliteIntegrationTest, TransactionCommit) {
    {
...
TEST_F(SqliteIntegrationTest, TransactionRollback) {
    {
        auto txn = db->begin_transaction();
...
TEST_F(SqliteIntegrationTest, NullAndNotNullFilters) {
...
TEST_F(SqliteIntegrationTest, SqlFunctionsFilter) {
...
TEST_F(SqliteIntegrationTest, InnerJoinAndLeftJoin) {
...
TEST_F(SqliteIntegrationTest, UpsertEntity) {
...
TEST_F(SqliteIntegrationTest, SetOperations) {
...
TEST_F(SqliteIntegrationTest, MultiTableJoins) {
...
TEST_F(SqliteIntegrationTest, CommonTableExpressions) {
    db->insert(users_table, User{0, "Alice", "alice@test.com", 30});
    db->insert(users_table, User{0, "Bob", "bob@test.com", 25});
    db->insert(users_table, User{0, "Charlie", "charlie@test.com", 40});

    // Execute query with CTE (WITH active_adults AS (...) SELECT * FROM active_adults WHERE age >= 30)
    auto prep = db->connection().prepare(
        "WITH active_adults AS (SELECT id, name, age FROM users WHERE age >= 25) "
