// ============================================================================
// IBM Informix Performance Benchmarks: Raw ODBC vs cpplinq ORM
// Scales: 1,000, 10,000, 100,000 rows
// ============================================================================
//
// Compares data access layers against the same IBM Informix instance:
//   1. Raw ODBC    — Direct ODBC 3.x API calls (no ORM overhead)
//   2. cpplinq ORM — Full ORM: AST -> SqlGenerator -> InformixConnection (ODBC) -> RowMapper
//
// Environment variables:
//   CPPLINQ_INFORMIX_ODBC or CPPDB_INFORMIX_ODBC — Informix ODBC DSN or connection string
//
// ============================================================================
...
// ============================================================================
// Environment variable helpers & connection string normalization
// ============================================================================

static std::string get_informix_conn_str() {
    const char* env = std::getenv("CPPLINQ_INFORMIX_ODBC");
    if (env && env[0] != '\0') return std::string(env);
    const char* env2 = std::getenv("CPPDB_INFORMIX_ODBC");
    return (env2 && env2[0] != '\0') ? std::string(env2) : "";
}

static std::string ensure_informix_conn_params(std::string conn_str) {
...
public:
    void SetUp(benchmark::State& state) override {
        auto conn_str = get_informix_conn_str();
        if (conn_str.empty()) {
            state.SkipWithError("CPPLINQ_INFORMIX_ODBC / CPPDB_INFORMIX_ODBC not set");
            return;
        }
        try {
            db = std::make_unique<DbContext<informix>>(conn_str);
            ensure_tables();
            clear_tables();
    ...
    std::unique_ptr<DbContext<informix>> db;
};
...
public:
    void SetUp(benchmark::State& state) override {
        ...
        try {
            ...
            std::string full_cs = ensure_informix_conn_params(conn_str);
    ...
    void insert_users_bulk(const std::vector<BenchUser>& users) {
        ...
        SQLSetConnectAttr(hdbc_, SQL_ATTR_AUTOCOMMIT, reinterpret_cast<SQLPOINTER>(SQL_AUTOCOMMIT_OFF), SQL_IS_UINTEGER);
